...
import { IProduct } from "../../../../prisma/testContent";

export default function Card({ product }: { product: IProduct }) { // { product: IProduct }
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      ...
      <div className="pt-6">
        ...
        <Link href={`shop/product/${product.id}`} className="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white">{product.name}</Link>
        ...
        <ul className="mt-2 flex items-center gap-4">
          <li className="flex items-center gap-2">
            ...
            {product.tags.map((tag: string, index: number) => (
              <p key={index} className="text-sm font-medium text-gray-500 dark:text-gray-400">{tag}</p>
            ))} 
          </li>
        </ul>
        ...
        <div className="mt-4 flex items-center justify-between gap-4">
          ...
          <button type="button"
            ...
            <svg className="-ms-2 me-2 h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
              fill="none" viewBox="0 0 24 24">
              <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
